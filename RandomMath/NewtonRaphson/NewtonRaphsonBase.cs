...
namespace RandomMath.NewtonRaphson
{
	/// <summary>
	...
	public abstract class NewtonRaphsonBase
	{
		...
		{
			double[] r = VectorMath.Subtract(yDesired, y);
			...
			public void Solve()
		{
			...
			while (flag == 0)
			{
				...
				double[] xStep = MatrixMath.LinSolve(T, rPrevious);

				xCurrent = VectorMath.Add(xPrevious, xStep);
                ...
                if (errorCurrent > errorPrevious) {
					...
					*/
                    ...
                    while ((miniCurrentIt <= 10) && (errorCurrent > errorPrevious)) {
                        xStep = VectorMath.ScalarMultiply(0.05, xStep);
                        if (miniCurrentIt >= 9) {
                            xStep = VectorMath.ScalarMultiply(-1.0, xStep);}
