...
namespace RandomMath.ODESystemSolver
{
    /// <summary>
    ...
    public class MatrixExponential
	{
		...
		public MatrixExponential(double [,] A, double xf, int maxIterations, double maxError, bool useTaylor = true)
		{
		...
		public double [,] Solve(double x){
			...
			double[,] Unsquared = useTaylor ? ExponentialTaylorsSeries(AScaled, x) : ExponentialPadeApproximation(AScaled, x);
		...
		private int FindScalingFactor(double [,] A, double xf){
			...
			int j = 0;
			double Norm = MatrixMath.PNorm(2, A);
		...
		private double [,] Scale(double [,] A, int j){
			...
			return MatrixMath.ScalarMultiply(scalingFactor, A);
		}
